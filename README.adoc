...
In this demo we'll take an introductionary look at how we can use Spring GraphQL in our Java applications.
...
To start of we'll need to add the `reactor-test` dependency:
...
And let's add an easy to test to check we receive Kent Beck's 9 books. (the API itself offers us a lot more options!)
